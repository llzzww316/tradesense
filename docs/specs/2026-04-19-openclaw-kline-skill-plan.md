...
> **归档说明（2026-05）**：本文为 2026-04-19 实施任务的**历史计划**，文中嵌入的 `SKILL.md` 模板片段仅作过程记录。当前以仓库内实装为准：`skills/tradesense-kline/SKILL.md`、`docs/openclaw-integration.md`。文中「掘金」等表述在当时成立，**当前 K 线来源为通达信 VIPDOC（`data_provider.py` + tdxpy）**。
...
description: 通过 TradeSense MCP 调用本仓库 HTTP API 获取 K 线（OHLCV）；须先启动本仓库 HTTP 服务（数据来自本机通达信离线文件）。
---
...
用户询问 **K 线、蜡烛图、OHLC、历史行情、复盘数据、某品种走势**，且数据源应为 **TradeSense 本机 HTTP 服务（通达信离线数据经后端）** 时：必须使用 MCP 工具，不要用 shell 随意抓取网页或未说明来源的数据。
...
- **空数据 / 本地行情缺失**：按仓库 `README.md` 检查通达信数据目录、`symbols.json` 与 `TDX_DIR`。
