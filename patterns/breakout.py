...
def detect_breakout(
    ...
    return {
        ...
        "detail": (f"突破阻力{resistance:.2f}, "
                   ...
                   f"量比{cur.volume / avg_vol:.1f}x" if avg_vol > 0 else f"突破阻力{resistance:.2f}"),
    }
...
def detect_breakout_pullback(
    ...
    # ---- 找突破事件 ----
    # 在过去 lookback 日内，哪一天发生了突破？
    # 定义突破：收盘价 > 前20日最高价
    breakout_day = None
    breakout_price = None  # 突破位（阻力位 = 现支撑位）
    ...
    for i in range(len(bars) - lookback, len(bars) - 2):
        ...
        if bars[i].close > prev_high and bars[i].close > bars[i].open:
            breakout_day = i
    ...
    # 突破日的最高价
    breakout_high = max(b.high for b in bars[breakout_day:breakout_day + 1])
